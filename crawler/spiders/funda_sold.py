...
class FundaSoldSpider(scrapy.Spider):
    ...
    def _sold_url(slug: str, page: int) -> str:
        base = f"https://www.funda.nl/koop/utrecht/{slug}/verkocht/"
    ...
    def parse_search_page(self, response, neighborhood: str, slug: str, page: int):
        ...
        logger.info(
            "Sold: neighborhood %s slug %s page %d: %d listings (total %s)",
        ...
        for listing in listings:
            ...
            yield scrapy.Request(
                ...
                cb_kwargs={"neighborhood": neighborhood, "funda_id": funda_id, "listing": listing},
            )
    ...
    def parse_listing(self, response, neighborhood: str, funda_id: str, listing: dict):
        item = FundaForSaleSpider._listing_to_item(listing, neighborhood)
        if item is None:
            return
        item["status"] = "sold"
